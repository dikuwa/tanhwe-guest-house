...
type RoomAmenity = { id: string; roomId: string; amenity: string };
type Room = {
  id: string; name: string; slug: string; type: string; pricePerNight: number;
  availableUnits: number; maxGuests: number; status: string;
  images: RoomImage[]; amenities: RoomAmenity[];
};
...
export function RoomsTable({ initial }: { initial: Room[] }) {
              ...
              return (
                <tr key={room.id} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50">
                  <td className="px-4 py-3">
                    {primaryImage ? (
                      <div className="relative size-10 overflow-hidden rounded-md bg-neutral-100">
                        <Image src={primaryImage.imageUrl} alt="" fill sizes="40px" className="object-cover" />
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-4">
                    <Link href={`/admin/rooms/${room.id}/edit`} className="font-medium text-neutral-800 hover:text-primary transition-colors">
                      {room.name}
                    </Link>
                    ...
                    {room.amenities.length > 0 && (
                      <div className="mt-1 flex flex-wrap gap-1">
                        {room.amenities.slice(0, 3).map((a) => (
                          <span key={a.id} className="inline-block rounded-full bg-neutral-100 px-2 py-0.5 text-[10px] text-neutral-500">
                            {a.amenity}
                          </span>
                        ))}
                        {room.amenities.length > 3 && (
                          <span className="text-[10px] text-neutral-400">+{room.amenities.length - 3}</span>
                        )}
                      </div>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-4 text-right">
                    <div className="flex justify-end gap-1">
                      <Button variant="outline" size="sm" render={<Link href={`/admin/rooms/${room.id}/edit`} />}>
                        Edit
                      </Button>
                      <DeleteRoomButton id={room.id} name={room.name} onDeleted={handleDeleted} />
                    </div>
                  </td>
                </tr>
              );
